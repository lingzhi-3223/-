...
stock=ts.get_hist_data('000777','2017-08-24',end='2018-08-24')#读取一支股票
stock=stock.sort_index(0)  # 将数据按照日期排序
data=stock.loc[:,['high','low','open','close']].values

dataCount=np.array(stock['high']).size#计算行数
'''
f=open('dataset/300104.csv')
df=pd.read_csv(f)     #读入股票数据
data=df.loc[:,['high','low','open','close']].values  #取第2-6列
'''


#获取训练集
def get_train_data(batch_size=60,time_step=1,train_begin=0,train_end=50):
    batch_index=[]
...
#获取测试集
def get_test_data(time_step=1,test_begin=50):
...
def train_lstm(batch_size=60,time_step=1,train_begin=50,train_end=200):
...
def prediction(time_step=1):
    ...
    with tf.Session() as sess:
        ...
        #得到之后100个预测结果
        for i in range(100):
            next_seq=sess.run(pred,feed_dict={X:[test_x[prev_seq]]})
            test_predict.extend(next_seq[-1])
