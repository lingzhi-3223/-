...
def train_lstm(batch_size=60,time_step=1,train_begin=0,train_end=50):
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        ...
        plt.show()
        num=np.array(test_predict)
        num=num.astype(np.float)
        num = pd.DataFrame(num,columns=['predict'])
        num.to_csv('E:/PythonText/PythonWeb/Stock_Show/02.csv')
